              ...
              SourceTerm&             sourceterm)
{
  ...
  for (time = 0.0; time < globals.endTime; time += globals.maxTimestep) {
    ...
    */
    ...
    MPI_Irecv(inbuf_x[1], locals.elts_size[0]*NUMBER_OF_DOFS, MPI_DOUBLE, locals.adj_list[DOWN], tag + locals.adj_list[DOWN], MPI_COMM_WORLD, &reqs[DOWN+4]);

	MPI_Isend(outbuf_y[0], locals.elts_size[1]*NUMBER_OF_DOFS, MPI_DOUBLE, locals.adj_list[LEFT], tag + locals.rank, MPI_COMM_WORLD, &reqs[LEFT]);
    MPI_Irecv(inbuf_y[0], locals.elts_size[1]*NUMBER_OF_DOFS, MPI_DOUBLE, locals.adj_list[LEFT], tag + locals.rank, MPI_COMM_WORLD, &reqs[LEFT+4]);
    MPI_Isend(outbuf_y[1], locals.elts_size[1]*NUMBER_OF_DOFS, MPI_DOUBLE, locals.adj_list[RIGHT], tag + locals.adj_list[RIGHT], MPI_COMM_WORLD, &reqs[RIGHT]);
    MPI_Irecv(inbuf_y[1], locals.elts_size[1]*NUMBER_OF_DOFS, MPI_DOUBLE, locals.adj_list[RIGHT], tag + locals.adj_list[RIGHT], MPI_COMM_WORLD, &reqs[RIGHT+4]);
